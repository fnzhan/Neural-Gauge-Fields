...
import timeit
import torch
from models.Field import TriPlane
...
def test(args):
    ...
    if args.render_train:
        ...
        PSNRs_test = evaluation(train_dataset, field, args, f'{logfolder}/imgs_train_all/',
                                N_vis=-1, N_samples=-1, white_bg = white_bg, device=device, infoinv=infoinv)
...
def renderer(rays, field, chunk=1024, N_samples=-1, white_bg=True, is_train=False, device='cuda', infoinv=True):
    rgbs, depth_maps = [], []
    ...
    for chunk_idx in range(N_rays_all // chunk + int(N_rays_all % chunk > 0)):
        ...
        output = field(rays_chunk, is_train=is_train, white_bg=white_bg, N_samples=N_samples, infoinv=infoinv)
...
def evaluation(test_dataset, field, args, savePath=None, N_vis=5, prtx='', N_samples=-1,
               white_bg=False, compute_extra_metrics=True, device='cuda', infoinv=True):
    ...
    for idx, samples in tqdm(enumerate(test_dataset.all_rays[0::img_eval_interval]), file=sys.stdout):
        ...
        rgb_map, depth_map = renderer(rays, field, chunk=4096, N_samples=N_samples, white_bg=white_bg, device=device, infoinv=infoinv)
...
def evaluation_path(test_dataset, field, c2ws, savePath=None, N_vis=5, prtx='', N_samples=-1,
    ...
    for idx, c2w in tqdm(enumerate(c2ws)):
        ...
        rgb_map, depth_map = renderer(rays, field, chunk=8192, N_samples=N_samples, white_bg=white_bg, device=device, infoinv=infoinv)
...
def train(args):
    ...
    for iteration in pbar:
        ...
        output = field(rays_train, is_train=True, white_bg=white_bg, N_samples=nSamples, infoinv=infoinv)
        rgb_map = output['rgb_map']
        ...
        if iteration % args.vis_every == args.vis_every - 1 and args.N_vis!=0:
            PSNRs_test = evaluation(test_dataset, field, args, f'{logfolder}/imgs_vis/', N_vis=args.N_vis,
                                    prtx=f'{iteration:06d}_', N_samples=nSamples, white_bg = white_bg, compute_extra_metrics=False, infoinv=infoinv)
        ...
        if iteration in update_AlphaMask_list:
            ...
            field.updateAlphaMask(tuple(reso_mask), infoinv=infoinv)
