...
class Base(torch.nn.Module):
    ...
    def transform(self, xyz_sampled):
        pass
    ...
    def compute_alpha(self, xyz_locs, length=1, infoinv=True):
        ...
        if alpha_mask.any():
            ...
            alpha_xy, alpha_yz, alpha_xz = self.transform(xyz_sampled)
            alpha_density = self.compute_density(alpha_xy, alpha_yz, alpha_xz, infoinv=infoinv)
    ...
    @torch.no_grad()
    def getDenseAlpha(self,gridSize=None, infoinv=True):
        ...
        for i in range(gridSize[0]):
            alpha[i] = self.compute_alpha(dense_xyz[i].view(-1,3), self.stepSize, infoinv=infoinv).view((gridSize[1], gridSize[2]))
        return alpha, dense_xyz

    @torch.no_grad()
    def updateAlphaMask(self, gridSize=(200,200,200), infoinv=True):

        alpha, dense_xyz = self.getDenseAlpha(gridSize, infoinv=infoinv)
    ...
    def forward(self, rays_chunk, white_bg=True, is_train=False, N_samples=-1, infoinv=True):
        ...
        if valid_ray.any():
            xyz_sampled = self.normalize_coord(xyz_sampled)
            valid_xy, valid_yz, valid_xz = self.transform(xyz_sampled[valid_ray])
            valid_density = self.compute_density(valid_xy, valid_yz, valid_xz, infoinv=infoinv)
        ...
        if rgb_mask.any():
            valid_rgb = self.compute_rgb(xy[rgb_mask], yz[rgb_mask], xz[rgb_mask], viewdirs[rgb_mask], infoinv=infoinv)
