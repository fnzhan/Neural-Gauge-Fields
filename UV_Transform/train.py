...
from data.dtu import create_data_loader, create_dataset
...
def test(model, dataset, visualizer, opt, test_steps):
    ...
    for i in range(opt.test_num):
        ...
        for k in range(0, height * width, chunk_size):
            # print("chunk {}/{}".format(k // chunk_size + 1, height * width // chunk_size))
            start = k
...
def main():
    ...
    if opt.resume_dir:
        ...
        states = torch.load(os.path.join(resume_dir, "{}_states.pth".format(opt.resume_epoch)))
    ...
    while not stop_iteration:
        ...
        for i, data in enumerate(data_loader):
            ...
            if total_steps and total_steps % opt.print_freq == 0:
                print("Total iterations {}/{}".format(total_steps, opt.niter + opt.niter_decay))
        ...
        print("{}: End of epoch {} \t Time Taken: {} sec".format(opt.name, epoch, time.time() - epoch_start_time,))
        epoch += 1
