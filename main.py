...
def renderer(rays, field, chunk=1024, N_samples=-1, white_bg=True, is_train=False, device='cuda'):
    ...
    for chunk_idx in range(N_rays_all // chunk + int(N_rays_all % chunk > 0)):
        ...
        output = field(rays_chunk, is_train=is_train, white_bg=white_bg, N_samples=N_samples)
...
def evaluation(test_dataset, field, args, savePath=None, N_vis=5, prtx='', N_samples=-1,
    ...
    for idx, samples in tqdm(enumerate(test_dataset.all_rays[0::img_eval_interval]), file=sys.stdout):
        ...
        rgb_map, depth_map = renderer(rays, field, chunk=1024, N_samples=N_samples, white_bg=white_bg, device=device)
...
def train(args):
    ...
    if args.ckpt is not None:
        ...
        field = eval(args.model_name)(aabb, reso_cur, device, near_far=near_far, alphaMask_thres=args.alpha_mask_thre,
                    distance_scale=args.distance_scale, step_ratio=args.step_ratio, transform_type=args.transform_type)
    ...
    for iteration in pbar:
        ...
        # loss_reg_L1 = field.density_L1() * 8e-5.
        ...
        #     uv = target_gauge.view(-1, 2)[:, :].detach().cpu().numpy()
        #     # plt.plot(uv[:, 0], uv[:, 1], markersize=4.0, color=(0.8, 0., 0.))
        #     plt.scatter(uv[:, 0], uv[:, 1], color=(0.8, 0., 0.), s=20)
        #     plt.savefig(f'{logfolder}/uv_{iteration:06d}.png')
        #     plt.close()

            # if 'target_gauge_feat' in output:
            #     inv = output['target_gauge_feat'].view(-1, 3)[:, :].detach().cpu().numpy()
            #     fig = plt.figure()
            #     ax = fig.add_subplot(projection='3d')
            #     ax.scatter(inv[:, 0], inv[:, 1], inv[:, 2])
            #     plt.savefig(f'{logfolder}/inv_{iteration:06d}.png')
            #     plt.close()
        #
        # if iteration % 1000 == 0:
        #
        #     grid = torch.tensor(generate_grid(2, 800)).float().to(device)
        #     viewdir = [-0.0798,  0.6545, -0.7519]
        #         # [-0.4318,  0.4350, -0.7902]
        #         # [-0.0798,  0.6545, -0.7519]
        #         # [-0.4318,  0.4350, -0.7902]# [0.5355, -0.8693, -0.1492]  # [0, 0, 1] 0.6937, -0.5344, -0.4829
        #     viewdir = (torch.tensor(viewdir).float().to(device).expand(grid.shape[:-1] + (3,)))
        #
        #     rgb = field.compute_uv(grid, viewdir)
        #     rgb = rgb.detach().cpu().numpy()
        #     rgb8 = to8b(rgb)
        #     # filename = f'/CT/Multimodal-NeRF/work/Neural-Gauge-Fields/uv_rgb.png'
        #     imageio.imwrite(f'{logfolder}/texture_{iteration:06d}.png', rgb8)
        ...
        if iteration % 10000 == 0:
            field.save(f'{logfolder}/{args.expname}.th')
