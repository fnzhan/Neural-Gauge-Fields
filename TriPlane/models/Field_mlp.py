...
class TensoRF(Base):
    ...
    def init_model(self, res=256, dim=64, scale=0.1, device=None):
        ...
        gauge_res = 256
        self.gauge_xy = torch.nn.Parameter(torch.zeros((1, 4, gauge_res, gauge_res), device=device))
        self.gauge_yz = torch.nn.Parameter(torch.zeros((1, 4, gauge_res, gauge_res), device=device))
        self.gauge_xz = torch.nn.Parameter(torch.zeros((1, 4, gauge_res, gauge_res), device=device))

        self.gauge_z = torch.nn.Parameter(torch.zeros((1, 4, gauge_res, 1), device=device))
        self.gauge_x = torch.nn.Parameter(torch.zeros((1, 4, gauge_res, 1), device=device))
        self.gauge_y = torch.nn.Parameter(torch.zeros((1, 4, gauge_res, 1), device=device))
        self.gauge_decoder = gauge_decoder().to(device)
    ...
    def get_optparam_groups(self, lr_init_spatialxyz=0.02, lr_init_network=0.001):
        grad_vars = [
                     ...
                     {'params': self.gauge_decoder.parameters(), 'lr': lr_init_network*0.1},
        ]
        return grad_vars
    ...
    def compute_gauge(self, valid_xyz, iteration=0):
        ...
        if iteration > 3000:
            fxy = F.grid_sample(self.gauge_xy, xy.view(1, N, 1, 2), align_corners=True).view(-1, xy.shape[0]).permute(1, 0)
            fyz = F.grid_sample(self.gauge_yz, yz.view(1, N, 1, 2), align_corners=True).view(-1, yz.shape[0]).permute(1, 0)
            fxz = F.grid_sample(self.gauge_xz, xz.view(1, N, 1, 2), align_corners=True).view(-1, xz.shape[0]).permute(1, 0)
            plane_feat = torch.cat((fxy, fyz, fxz), 1)
            ...
            fz = F.grid_sample(self.gauge_z, z_, align_corners=True).view(-1, z_.shape[1]).permute(1, 0)
            fx = F.grid_sample(self.gauge_x, x_, align_corners=True).view(-1, x_.shape[1]).permute(1, 0)
            fy = F.grid_sample(self.gauge_y, y_, align_corners=True).view(-1, y_.shape[1]).permute(1, 0)

            line_feat = torch.cat((fz, fx, fy), 1)
            # print(line_feat.shape, plane_feat.shape)

            gauge_feat = plane_feat * line_feat
            # gauge_feat = gauge_feat.view(xy.shape[0], 3, 4).sum(-1)
            gauge = self.gauge_decoder(gauge_feat)
            # self.gauge_decoder
            target_xy, target_yz, target_xz = xy + gauge[:, :2], yz + gauge[:, 1:], xz + gauge[:, ::2]
            target_z, target_x, target_y = z + gauge[:, 2:], x + gauge[:, :1], y + gauge[:, 1:2]
