...
def main():
    ...
    if opt.primitive_type == "sphere" and opt.target_texture == 'None':
        ...
        filepath = os.path.join(rootdir, opt.output_dir, f"cube_view.png")
        os.makedirs(os.path.dirname(filepath), exist_ok=True)
        Image.fromarray((texture * 255).astype(np.uint8)).save(filepath)

        texture = net_texture._export_sphere(512, viewdir) ** (1 / 2.2)
        texture = texture.clamp(0, 1).data.cpu().numpy()
        filepath = os.path.join(rootdir, opt.output_dir, f"sphere_view.png")
        os.makedirs(os.path.dirname(filepath), exist_ok=True)
        Image.fromarray((texture * 255).astype(np.uint8)).save(filepath)
    ...
    elif opt.primitive_type == "square" and opt.target_texture == 'None':
        texture = net_texture.export_textures(512, viewdir) ** (1 / 2.2)
        texture = texture.clamp(0, 1).data.cpu().numpy()
        filepath = os.path.join(rootdir, opt.output_dir, f"square_view.png")
        os.makedirs(os.path.dirname(filepath), exist_ok=True)
        Image.fromarray((texture * 255).astype(np.uint8)).save(filepath)
